...
import { getProperty, OrDefault, OrFail, textEllipsis } from "../../../src/core/lib/helper.lib"
...
describe("helper.lib.ts", function(){
    ...
    describe("#OrDefault<T>", function(){
        ...
        it("should return the default value if arg is undefined", function(done){
            expect(OrDefault(undefined,"DefaultValue")).to.equal("DefaultValue");
            done();
        })
    })
    ...
    describe("#textEllipsis", function(){
        ...
        it( "should return string ending with ellipsis if side = 'end'", function(done){
            expect(textEllipsis("Long test string", 5, {side: "end", ellipsis: '...'}).endsWith("...")).to.be.true
            done()
        })

        it( "should return string starting with ellipsis if side = 'start'", function(done){
            expect(textEllipsis("Long test string", 5, {side: "start", ellipsis: '...'}).startsWith("...")).to.be.true
            done()
        })
    })
