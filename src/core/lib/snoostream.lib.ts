...
export default class SnooStream {
    ...
    constructor(options: Snoowrap | Snoowrap.SnoowrapOptions, drift: number = 0) {
        this.snoowrap = isSnoowrap(options) ? (options as Snoowrap) : new Snoowrap(options as Snoowrap.SnoowrapOptions);
        this.startTime = Math.floor(Date.now() / 1000);
        this.drift = drift;
    }

    postStream(pollFn: tPollFn, subreddit: string = 'all', opts: IPostStreamOptions = {}): Pollify {
        ...
        poll.on('data', data => {
            ...
            data.filter((post: Snoowrap.Comment | Snoowrap.Submission) => post.created_utc >= this.startTime - this.drift)
                //.forEach( (post: Snoowrap.Comment | Snoowrap.Submission) => this.parse(post, poll, opts.regex))
                .forEach((post: Snoowrap.Comment | Snoowrap.Submission) => poll.emit('post', post))
        })
    ...
    private dedupe(batch: any[], cacheObj: IPostStreamCacheObject): any[] {
        const diff = batch.filter((entry: any) => cacheObj.cache.every(oldEntry => entry.id !== oldEntry.id))
